...
return slicedAgain;
    ...
    document.documentElement.style.setProperty("--colorDark1HslBase", sliceHsl(color));
    document.documentElement.style.setProperty("--colorDark1base", sliceHslAlpha(color));
    ...
    document.documentElement.style.setProperty("--colorDark2base", sliceHslAlpha(color2));
    ...
    document.documentElement.style.setProperty("--colorLight2base", sliceHslAlpha(color3));
    ...
    document.documentElement.style.setProperty("--colorLight1base", sliceHslAlpha(color4));
    document.documentElement.style.setProperty("--colorLight1HslBase", sliceHsl(color4));
    ...
    document.documentElement.style.setProperty("--colorLight3base", sliceHslAlpha(color6));
...
if(imgSrc ){
  ...
  return (
    ...
    <div className="w-full sentence-container fade-effect-quick">
      {loading ? (
        ...
        <div>
          ...
          <div className="flex flex-col min-w-[40em] gap-5 p-5 glass-box">
            ...
            <div className="flex gap-5">
              <p className=" whitespace-nowrap">Background gradient: </p>
    ...
    <div className="w-5 h-5 bg-blues-900"></div>
          ...
          <ChromePicker color={color} onChange={(x) => {
            let s = ((x.hsl.s).toFixed(2)) * 100;
            let l = ((x.hsl.l).toFixed(2)) * 100;
           let hsl = `hsla(${Math.round(x.hsl.h)},${Math.round(s)}%,${Math.round(l)}%,1)`;
              setColor(hsl)}} />
            </div>
            <div className="flex flex-col items-center">
          <p>Color 1 dark</p>
          <div className="h-8 rounded-xl w-[12.5rem] bg-t-bd"></div>
              ...
              <ChromePicker color={color2} onChange={(x) => {
            let s = ((x.hsl.s).toFixed(2)) * 100;
            let l = ((x.hsl.l).toFixed(2)) * 100;
           let hsl = `hsla(${Math.round(x.hsl.h)},${Math.round(s)}%,${Math.round(l)}%,1)`;
              setColor2(hsl)}} />
          ...
          <div className="h-8 rounded-xl w-[12.5rem] bg-t-bpop"></div>
              ...
              <ChromePicker color={color5} onChange={(x) => {
            let s = ((x.hsl.s).toFixed(2)) * 100;
            let l = ((x.hsl.l).toFixed(2)) * 100;
           let hsl = `hsla(${Math.round(x.hsl.h)},${Math.round(s)}%,${Math.round(l)}%,1)`;
           setColor5(hsl)} }/>
...
{/* <div className="flex rounded-lg">
    ...
    <div className="w-5 h-5 bg-pinks-900"></div>
          ...
          <div className="h-8 rounded-xl w-[12.5rem] bg-clear-pl5"></div> */}
              <ChromePicker color={color4} onChange={(x) => {
            let s = ((x.hsl.s).toFixed(2)) * 100;
            let l = ((x.hsl.l).toFixed(2)) * 100;
           let hsl = `hsla(${Math.round(x.hsl.h)},${Math.round(s)}%,${Math.round(l)}%,1)`;
              setColor4(hsl)}} />
          ...
          <div className="h-8 rounded-xl w-[12.5rem] bg-t-pd"></div>
              ...
              <ChromePicker color={color3} onChange={(x) => {
            let s = ((x.hsl.s).toFixed(2)) * 100;
            let l = ((x.hsl.l).toFixed(2)) * 100;
           let hsl = `hsla(${Math.round(x.hsl.h)},${Math.round(s)}%,${Math.round(l)}%,1)`;
           setColor3(hsl)} }/>
            ...
            <div className="flex flex-col items-center">
          <p >Color 2 Medium <span className="text-xs">(minor color)</span></p>
          <div className="h-8 rounded-xl w-[12.5rem] bg-t-pm"></div>

              <ChromePicker color={color6} onChange={(x) => {
            let s = ((x.hsl.s).toFixed(2)) * 100;
            let l = ((x.hsl.l).toFixed(2)) * 100;
           let hsl = `hsla(${Math.round(x.hsl.h)},${Math.round(s)}%,${Math.round(l)}%,1)`;
           setColor6(hsl)} }/>
