...
function ChipYellow({ icon, text, cN = " w-fit ", updateButton, reset, load }) {
  ...
  return (
    <button
      className={
        ...
        cN +
        (clicked
          ? " border-4 border-yellow-700 bg-yellow-500 text-white"
    ...
    >
      {icon} {text}
      {clicked ? (
        <span className="absolute flex items-center justify-center w-5 h-5 leading-none text-center text-black bg-yellow-200 rounded-full -top-2 -left-2">
