...
function ChipSky({ icon, text, cN = " w-fit ", updateButton, reset, load }) {
  ...
  return (
    <button
      className={
        ...
        cN +
        (clicked
          ? " border-4 border-sky-700 bg-sky-400 text-white"
    ...
    >
      {icon} {text}
      {clicked ? (
        <span className="absolute flex items-center justify-center w-5 h-5 leading-none text-center text-black rounded-full bg-sky-200 -top-2 -left-2">
