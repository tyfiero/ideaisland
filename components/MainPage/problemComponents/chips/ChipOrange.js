...
function ChipOrange({ icon, text, cN = " w-fit ", updateButton, reset, load }) {
  ...
  return (
    <button
      className={
        ...
        cN +
        (clicked
          ? " border-4 border-orange-700 bg-orange-400 text-white"
    ...
    >
      {icon} {text}
      {clicked ? (
        <span className="absolute flex items-center justify-center w-5 h-5 leading-none text-center text-black bg-orange-200 rounded-full -top-2 -left-2">
