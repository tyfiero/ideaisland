...
function ChipTeal({ icon, text, cN = " w-fit ", updateButton, reset, load }) {
  ...
  return (
    <button
      className={
        ...
        cN +
        (clicked
          ? " border-4 border-teal-700 bg-teal-400 text-white"
          : "bg-teal-200 text-slate-700")
      }
      onClick={(e) => {
        setClicked(!clicked);
    ...
    >
      {icon} {text}
      {clicked ? (
        <span className="absolute flex items-center justify-center w-5 h-5 leading-none text-center text-black bg-teal-200 rounded-full -top-2 -left-2">
