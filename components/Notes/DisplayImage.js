...
function DisplayImage({type, mode}) {
      ...
      let pexelsWindow = (
        <div className=" flex items-center flex-col w-[35em] z-50">
          <div
            ...
            className="relative flex items-center w-full text-gray-600 bg-white border-2 border-gray-300 rounded-2xl"
          >
            <input
            ...
            <button
              // type="submit"
              onClick={() => {searchPexels(searchTerm)}}
  ...
  >
    ...
    <button
      ...
      className={mode === "edit" ? " fade-effect-turbo opacity-100 absolute top-2 left-6" : "group-hover:opacity-100 opacity-0 fade-effect-turbo transition absolute top-2 left-4"} 
    >
    ...
    {showImgPopUp && (
      ...
      >
        ...
        <input
          type="text"
          className="mx-5 mb-1 textarea-box textarea-tw !rounded-xl"
...
{/* {loading ? <Skeleton className=" object-cover !leading-loose !rounded-xl w-[12em] h-[8em] !p-0" />: <img
    ...
    placeholder="blur"
  />} */}
{imageLoading && <div className="z-0 w-full h-full">
    <Skeleton width={"100%"} height={"100%"} className=" w-full h-full object-cover !leading-loose !rounded-xl  !p-0 fade-effect-quick z-100"  baseColor="var(--colorSkeleton1)" highlightColor="var(--colorSkeleton2)"/>
    </div> }
...
setPosition(currentDocRedux.imgPosition);
      ...
      className={" w-full fade-effect-turbo " + (imageLoading ? " !hidden" : "")}
      src={imgSrc}
      alt="cover image"
    placeholder="blur"
