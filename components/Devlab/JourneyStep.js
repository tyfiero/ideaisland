...
function JourneyStep({
  ...
  const saveProgress = () => {
    ...
    if (!stepChanged) {
      ...
      //There are changes, but there is no entry for this step yet
      // console.log("changes and no entry");

      if (updated.results.length > 0) {
        updated.results = [...updated.results, entry];
      } else {
        updated.results.push(entry);
      }
      dispatch(currentJourneyAction(updated));
    }

    console.log(updated);
    // await dispatch(currentJourney(updated));
    // if (!props.changes) {
...
"
      >
        <div className="w-full max-w-[42rem] p-10 space-y-8  glass-box border-2 border-slate-200 bg-slate-200/60">
          ...
          <div className="relative flex flex-col items-center justify-between gap-3 p-3 problem-page fade-effect-quick min-h-[25em]">
            ...
            <div className=" glass-box border-slate-400 bg-white-400/40 min-h-[10em] min-w-[30em] p-5 my-2">
              <h3 className="heading text-blues-600 dark:text-blues-100">
                {text || "Question"}
              </h3>
            ...
            <div className="flex items-center justify-between w-full mt-3">
              <div className="relative group">
                <div
                  className={
                    "absolute transition duration-1000  !rounded-3xl -inset-1 bg-gradient-to-r from-t-pl via-t-pm to-t-pd blur-sm group-hover:opacity-100 group-hover:duration-200 animate-gradient-xy" +
                    (props.currentStep === 1
                ...
                <button
                  ...
                  className={
                    " px-3 h-[3em] glass-box text-white !border-white/0 bg-gradient-to-r from-clear-pl2 via-clear-pl4 to-clear-pl4  rounded-2xl right-[50px] flex items-center justify-center md:hover:scale-105 md:transition-transform md:active:scale-95 fade-effect cursor-pointer shadow-clear-pd2 shadow-xl m-1 drop-shadow-xl " +
                    (props.currentStep === 1
                      ? " opacity-0 cursor-default"
                      : " opacity-100 cursor-pointer ")
                  }
